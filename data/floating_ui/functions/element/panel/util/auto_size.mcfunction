#> floating_ui:element/panel/util/auto_size
# @internal

tag @s add floating_ui_this
execute on vehicle if entity @s[type=marker] run data modify entity @e[tag=floating_ui_this,limit=1,sort=nearest] item.tag.data.size set value [5.0f,5.0f]
...
execute on vehicle store result score width _ run data get entity @s item.tag.data.size[0] 0.5
execute on vehicle store result score height _ run data get entity @s item.tag.data.size[1] 0.5
