...
# - score delay floating_ui.temp = 0 动画播放延迟(t)
# - storage floating_ui:input animate.data 一个有效的动画数据

#立刻触发
execute if score delay floating_ui.temp matches ..0 run data modify storage floating_ui:temp anim set from storage floating_ui:input animate.data
execute if score delay floating_ui.temp matches ..0 run return run function floating_ui:element/control/animation/custom_do
...
scoreboard players operation inp int = delay floating_ui.temp
