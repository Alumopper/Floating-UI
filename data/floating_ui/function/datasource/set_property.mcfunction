...
# {path: xxx, value: xxx}
execute store result score floating_ui.temp int run function floating_ui:datasource/get_or_create_data_id with storage floating_ui:temp binding
#设置值
function floating_ui:datasource/set_value with storage floating_ui:temp binding
execute if score isChanged floating_ui.temp matches 0 run return 0
#通知所有UI刷新
