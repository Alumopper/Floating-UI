...
execute as 1bf52-0-0-0-2 on origin run data modify entity 1bf52-0-0-0-2 Thrower set from entity @s item.components.minecraft:custom_data.childPoint[0]
...
execute if score isLookedAt _ matches 1 as 1bf52-0-0-0-2 on origin run function floating_ui:interact/move/trigger with storage floating_ui:temp arg 
...
#链式检测并传递moveIn事件
execute as 1bf52-0-0-0-2 on origin run function floating_ui:interact/move_in/trigger with storage floating_ui:temp arg
