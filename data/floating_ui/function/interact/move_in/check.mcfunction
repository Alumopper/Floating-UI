...
#之前是否被观察
scoreboard players set isLookedAt floating_ui.temp 0
execute as 1bf52-0-0-0-2 on origin if entity @s[tag=floating_ui_legacy_lookedAt] run scoreboard players set isLookedAt floating_ui.temp 1
...
execute if score isLookedAt floating_ui.temp matches 1 as 1bf52-0-0-0-2 on origin on passengers run function floating_ui:interact/move/trigger with storage floating_ui:temp arg 
execute if score isLookedAt floating_ui.temp matches 1 run return 0
