...
#判断是那个按下
execute if entity @s[advancements={floating_ui:left_click=true}] run scoreboard players set click_status floating_ui.temp 1
advancement revoke @s[advancements={floating_ui:left_click=true}] only floating_ui:left_click
execute if entity @s[advancements={floating_ui:right_click=true}] run scoreboard players set click_status floating_ui.temp -1
...
#链式检测并传递click事件
execute as 1bf52-0-0-0-2 on origin on passengers run function floating_ui:interact/click/trigger with storage floating_ui:temp arg
